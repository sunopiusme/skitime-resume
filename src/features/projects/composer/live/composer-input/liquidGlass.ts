...
export type GlassTuning = {
  ...
  scaleRatio: number;
  /** Предразмытие подложки, stdDeviation. */
  blur: number;
...
export function buildGlassFilterMarkup(
  ...
  const specUrl = generateSpecularMap(w, h, t.radius, clampedBezel * 2.5, Math.PI / 3);
  // scale = фактический максимум смещения в px. Ограничиваем
  // maxShift'ом, чтобы на компактной панели текст «уходил» в
  // стекло заметно, но без агрессивного разрыва.
  const scale = Math.min(maxDisp, t.maxShift) * t.scaleRatio;

  return [
    `<feGaussianBlur in="SourceGraphic" stdDeviation="${t.blur}" result="blurred_source"/>`,
