      ...
      <article className={caseStyles.article}>
        ...
        <section className={caseStyles.part}>
          <div className={caseStyles.column}>
            ...
            <div className={caseStyles.prose}>
              <p>
                Узкое место разработки сместилось:{" "}
                <em>медленной частью стало не написание кода, а его проверка</em>. Код агента
                выглядит уверенно, проходит базовые тесты и скрывает ошибки намерения, которые
                проявляются уже в продакшене.
              </p>
              <p>
                Существующие инструменты отвечают на вопрос, как сгенерировать код, но не отвечают
                на три вопроса ревьюера:{" "}
                <em>что агенту разрешено, что он делает прямо сейчас и почему его результат можно
                принять</em>.
              </p>
            </div>
          </div>
          ...
          <div className={caseStyles.column}>
            ...
            <div className={caseStyles.prose}>
              <p>
                ...
                на набор функций, а на{" "}
                <em>точки, где пользователь теряет контроль</em>: права, контекст, ход работы и
                diff. Документацию и пользовательское трение кодировал отдельно, потому что первая
                говорит, что продукт обещает, а второе показывает, где обещание не выдерживает
                практики.
              </p>
              <p>
                Повторяющееся трение везде одно:{" "}
                <em>автономность повышает скорость и одновременно цену ошибки</em>. Отсюда
                требование к интерфейсу, который даёт пользователю контроль до запуска, во время
                работы и после результата.
              </p>
            </div>
          </div>
        ...
        <section className={caseStyles.part}>
          <div className={caseStyles.column}>
            ...
            <div className={caseStyles.prose}>
              <p>
                <em>Агент стартует вслепую</em>, потому что параметры запуска рассеяны по
                конфигурациям и памяти пользователя: Codex распределяет контракт между AGENTS.md и
                облачным окружением, Claude Code опирается на permissions, hooks и MCP, а Kiro
                держит дисциплину через спеки.
              </p>
              <p>
                Композер собирает намерение в{" "}
                <em>исполняемый контракт</em>: задача, файлы, проект, ветка, окружение, модель и
                режим доступа находятся в одном кадре. Пользователь{" "}
                <em>останавливает агента до первого изменения</em>, а не разбирает последствия
                после готового diff.
              </p>
            </div>
          </div>
        ...
        <section className={caseStyles.part}>
          <div className={caseStyles.column}>
            ...
            <div className={caseStyles.prose}>
              <p>
                Работающий агент <em>остаётся чёрным ящиком</em>: пока он действует, интерфейс
                показывает либо спиннер, либо театр размышлений, и ни то, ни другое не помогает
                ревьюеру.
              </p>
              <p>
                Вместо потока размышлений нужен{" "}
                <em>операционный журнал</em>: что прочитано, какие команды запущены, где агент
                изменил файл и чем подтверждён результат. По такому следу{" "}
                <em>причинность восстанавливается без открытия десяти вкладок</em>.
              </p>
            </div>
          </div>
          ...
          <div className={caseStyles.column}>
            <div className={caseStyles.prose}>
              <p>
                <em>Сценарий вынесен из UI</em>, чтобы журнал не зависел от анимации.{" "}
        ...
        <section className={caseStyles.part}>
          <div className={caseStyles.column}>
            ...
            <div className={caseStyles.prose}>
              <p>
                ...
                результат и проверку в одну ленту, где{" "}
                <em>всё выглядит одинаково важным</em>.
              </p>
              <p>
                Вместо ленты нужна <em>диспетчерская</em>: линии работ, ограничения, пакет следов
                из команд, тестов, diff и снимков, а также{" "}
                <em>явная точка, где человек принимает или отклоняет результат</em>.
              </p>
            </div>
          </div>
          ...
          <div className={caseStyles.column}>
            <div className={caseStyles.prose}>
              <p>
                <em>Сайдбар здесь не меню чатов</em>. Он показывает работы в разных состояниях:
                агент ещё действует, diff ждёт ревью, задача уже закрыта. Сверху расположены
                навигация и проекты, снизу хвост списка и системный футер, между ними пустое поле
                страницы. Пауза оставлена специально. В ADE важнее{" "}
                <em>дистанция между задачами</em>, чем ещё один декоративный разделитель.
              </p>
            </div>
          </div>
          ...
          <div className={caseStyles.column}>
            <div className={caseStyles.prose}>
              <p>
                Сайдбар обязан отвечать на один вопрос:{" "}
                <em>что требует моего решения сейчас</em>. <code>running</code> означает, что
                агент ещё действует. <code>review</code> сообщает, что diff готов и нужен человек.{" "}
                <code>done</code> закрывает задачу.
              </p>
              <p>
                История сообщений вторична.{" "}
                <em>Важны задача, ветка, статус и доказательства</em>. Без них боковая панель
                превращается в архив разговоров, где невозможно отличить прогресс от шума.
              </p>
            </div>
          </div>
          ...
          <div className={caseStyles.column}>
            ...
            <div className={caseStyles.prose}>
              <p>
                ...
                MCP-сервер. Детерминированные сценарии проверяют{" "}
                <em>форму контроля, а не надёжность инфраструктуры</em>, и для концепта это
                честнее.
              </p>
              <p>
                Результат кейса состоит из трёх рабочих прототипов и критерия, который переносится
                на любую агентную среду:{" "}
                <em>интерфейс не просит верить агенту, а показывает, почему результат можно
                принять</em>.
              </p>
            </div>
          </div>
